class Map {
   ...
   */
  highlightPlacemark(targetPlacemark) {
    return new Promise((resolve, reject) => {
      if (targetPlacemark) {
...
const CENTER_COORDS = [55.76, 37.64];
const ZOOM_MULTIPLIER = 11;
const MAP_CONTAINER = "map";

const map = new Map(CENTER_COORDS, ZOOM_MULTIPLIER, MAP_CONTAINER);
ymaps
  .ready()
  .then(() => map.initialize())
  .then((map) => {
...
btns.forEach((btn) => {
  btn.addEventListener("click", (event) => {
    ...
    let coords = currentBtn.getAttribute("data-cords");
    map
      .resetHighlight()
      .then(() => map.transformCoordinates(coords))
      .then((cords) => map.findPlacemark(cords))
      .then((marker) => map.highlightPlacemark(marker));
